...
GCC_perc <- function(p, network, ensemble = 10) {
  ...
  res <- lapply(
    ...
    function(x) {
      ...
      mod_g_ER <- network - E(network)[removed]

      GCC <- max(components(mod_g_ER)$csize) / n
      err_GCC <- (GCC)**2
...
ER_results <- data.frame(matrix(ncol = 5, nrow = p_steps))
colnames(ER_results) <- c("p", "GCC_clean", "err_clean", "GCC_swap", "err_swap")


# We generate one random graph and apply different percolations,
# it's not the best solution but it's faster (needed)
g_ER <- sample_gnp(n, p = p2, directed = FALSE)
# Add ids to the vertices, better
V(g_ER)$name <- V(g_ER)
g_ER_clean <- sample_gnp(n, p = p2, directed = FALSE)
V(g_ER_clean)$name <- V(g_ER_clean)
...
ER_results$p <- p_range
ER_results$GCC_clean <- unlist(GCC)
ER_results$err_clean <- unlist(err_GCC)
...
for (q_node in which(degs %in% q_strategy, arr.ind = TRUE)) {
  ...
  q_node_neighbors <- neighbors(g_ER, V(g_ER)[q_node])
...
ER_results$GCC_swap <- unlist(GCC)
ER_results$err_swap <- unlist(err_GCC)
print("Done!")

# stopCluster(cl)

p1 <- ggplot(data = ER_results) +
...
ggsave(paste("ER_perc", n, sep = "_"), width = 5, height = 5, device = "png")
