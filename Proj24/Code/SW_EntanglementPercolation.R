...
GCC_perc <- function(p, network, ensemble = 10) {
  ...
  res <- lapply(
    ...
    function(x) {
      ...
      mod_g_SW <- network - E(network)[removed]

      GCC <- max(components(mod_g_SW)$csize) / n
      err_GCC <- (GCC)**2
...
SW_results <- data.frame(matrix(ncol = 5, nrow = p_steps))
colnames(SW_results) <- c("p", "GCC_clean", "err_clean", "GCC_swap", "err_swap")
...
for (i in seq.int(n - 1)) {
  new_edges <- which(runif(n - i) <= p2)
  if (length(new_edges) == 0) next()
  g_SW <- g_SW + edge(
    c(
      rbind(
        V(g_SW)$name[i],
        V(g_SW)$name[new_edges]
      )
    )
  )
}

g_SW_clean <- make_ring(n)
V(g_SW_clean)$name <- V(g_SW_clean)
# Adding random edges
for (i in seq.int(n)) {
  new_edges <- which(runif(n - i) <= p2)
  if (length(new_edges) == 0) next()
  g_SW_clean <- g_SW_clean + edge(
    c(
      rbind(
        V(g_SW_clean)$name[i],
        V(g_SW_clean)$name[new_edges]
      )
    )
  )
}
...
SW_results$p <- p_range
SW_results$GCC_clean <- unlist(GCC)
SW_results$err_clean <- unlist(err_GCC)
...
for (q_node in which(degs %in% q_strategy, arr.ind = TRUE)) {
  ...
  q_node_neighbors <- neighbors(g_SW, V(g_SW)[q_node])
...
SW_results$GCC_swap <- unlist(GCC)
SW_results$err_swap <- unlist(err_GCC)
print("Done!")

# stopCluster(cl)

p1 <- ggplot(data = SW_results) +
  ...
  labs(
    title = paste(
      "Percolation in SW net with and without Q-SW with N=",
      n,
      sep = ""
    ),
    x = "p",
...
ggsave(paste("SW_perc", n, sep = "_"), width = 5, height = 5, device = "png")
