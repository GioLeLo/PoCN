...
tot_perc <- function(p, ensemble = 10, DIAMETER = TRUE) {
  ...
  res <- lapply(
    seq_len(ensemble),
    function(x) {
      removed <- which(runif(length(E(g_ER))) > p, arr.ind = TRUE)
      mod_g_ER <- g_ER - E(g_ER)[removed]

      GCC <- max(components(mod_g_ER)$csize)
      err_GCC <- (GCC)**2
      diam <- diameter(mod_g_ER)
      err_diam <- (diam)**2
...
# We use the same (again) random graph g_ER
