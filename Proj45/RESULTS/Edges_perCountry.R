...
# icc <- c("AL","AD","AT","BY","BE","BA","BG","HR","CHLI","CZ","DK","EE","FO","FI","FR","DE","GI","GR","HU","IS","IE","IM","IT","RS","LV","LI","LT","LU","MK","MT","MD","MC","ME","NL","NO","PL","PT","RO","RU","SM","RS","SK","SI","ES","SE","CH","UA","GB","VA","RS")
# icc <- c("AL","AD","AT","BY","BE","BA","BG","HR","CHLI","DE","GI","GR","HU")
# icc <- c("HU","CZ","DK","EE","FO","FR","FI")
# icc <- c("HU","IS","IE","IM","IT","RS","LV","LI","LT","LU","MK","MT","MD","MC","ME","NL","NO","PL","PT","RO")
# icc <- c("RO","RU","SM","RS","SK","SI","ES","SE","CH","UA","GB","VA","RS")
icc <- c("BE")
...
accuracy <- 5
  ...
  if (dir.exists(file.path(root_path, icc))) {
    dir_path <- paste(root_path, icc, sep = "/")
    if (!file.exists(file.path(dir_path, "RailrdL.shp"))) {
      return(NULL)
    }
  } else {
    return(NULL)
...
paste(length(to_color) - length(V(railway)))
  ...
  # Obviously we have no time for that.

  n_stations <- length(to_color)

  max_length <- diameter(railway)

  conn <- mclapply(seq(n_stations - 1), function(node1) {
    print(paste("to do node", node1, "of country:", icc))
